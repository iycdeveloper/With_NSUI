...
class MembershipMemberListController extends GetxController {
  ...
  Future<bool> getMembershipList(BuildContext context, String batchId,
    ...
    // _membershipRequestList = await membershipDbRepo.getData(batchId);
    // if (_membershipRequestList.any((element) =>
    //  element.isIncMember!) ||
    //     _membershipRequestList.length > 1) {
    //   showAddOption = false;
    // }
    if (_membershipRequestList.length > 1) {
      showAddOption = false;
      update();
    }
    loadingPage = false;
  ...
  Future<bool> agrDownloadMembers({
    ...
    if (apiResponse.response != null &&
      ...
      if (responseDecoded['status'] == "SUCCESS") {
        ...
        _membershipRequestList = memberData.response.batchMember!;

        // _membershipRequestList = await membershipDbRepo.getData(batchId);
        _membershipRequestList.forEach((element) async {
          element.isSync = "1";
